...
# رمز التحقق الخاص بك لربط واتساب (أدخل رمزاً سرياً تطبقه في لوحة مطوري ميتا)
...
def receive_whatsapp_message():
    data = request.json
    logging.info( حمولة بيانات واتساب الواردة: {data} )
    
    try:
        # استخراج بيانات الرسالة من هيكل WhatsApp Cloud API القياسي
        ...
        if messages:
            ...
            sender_id = message_data.get("from") # رقم الهاتف المرسل
            msg_body = message_data.get("text", {}).get("body", "") # نص الرسالة
            
            # معالجة الإشارة عبر جسر الفريق
